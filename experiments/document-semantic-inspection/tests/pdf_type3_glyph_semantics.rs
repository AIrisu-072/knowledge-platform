...
fn type3_glyph_image_changes_pdfium_raster_and_semantic_fingerprint() {
    ...
    let red_result = PdfAdapter.inspect(&red, &InspectionProfile::default());
    let blue_result = PdfAdapter.inspect(&blue, &InspectionProfile::default());
    match (red_result, blue_result) {
        (Ok(red), Ok(blue)) => {
            assert_eq!(projection_text(&red.semantic_projection).trim(), "A");
            assert_eq!(projection_text(&blue.semantic_projection).trim(), "A");
            assert_ne!(
                fingerprint(&red.semantic_projection),
                fingerprint(&blue.semantic_projection),
                "an invoked image inside a Type 3 glyph is visible PDF image content"
            );
        }
        (Err(red), Err(blue)) => {
            assert_eq!(red.code(), ErrorCode::UnsupportedSemanticConstruct);
            assert_eq!(blue.code(), ErrorCode::UnsupportedSemanticConstruct);
        }
        (red, blue) => panic!(
...
fn projection_text(projection: &[u8]) -> String {
    let projection: Value = serde_json::from_slice(projection).expect("semantic projection JSON");
    projection["pages"][0]["text"]
        .as_str()
        .expect("page text")
        .to_owned()
}
