...
#[test]
fn referenced_list_level_text_changes_semantics_or_fails_closed() {
    let before = numbered_docx("%1.", 1);
    let after = numbered_docx("§%1)", 1);

    assert_numbering_is_referenced(&before, "%1.", 1);
    assert_numbering_is_referenced(&after, "§%1)", 1);
    assert_only_numbering_part_differs(&before, &after);
    assert_semantics_distinguish_or_fail_closed(
        &before,
        &after,
        "referenced w:lvlText visible marker",
    );
}

#[test]
fn referenced_list_start_changes_semantics_or_fails_closed() {
    let before = numbered_docx("%1.", 1);
    let after = numbered_docx("%1.", 3);

    assert_numbering_is_referenced(&before, "%1.", 1);
    assert_numbering_is_referenced(&after, "%1.", 3);
    assert_only_numbering_part_differs(&before, &after);
    assert_semantics_distinguish_or_fail_closed(
        &before,
        &after,
        "referenced w:start visible marker",
    );
}

fn numbered_docx(level_text: &str, start: u32) -> Vec<u8> {
    let mut parts = BTreeMap::from([
        ...
        (
            ...
            format!(
                r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="{WORDPROCESSINGML}"><w:body><w:p><w:pPr><w:numPr><w:ilvl w:val="0"/><w:numId w:val="42"/></w:numPr></w:pPr><w:r><w:t>same list item body text</w:t></w:r></w:p></w:body></w:document>"#
            )
            .into_bytes(),
        ),
    ]);
    ...
    parts.insert(
        NUMBERING_PART.into(),
        format!(
            r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:numbering xmlns:w="{WORDPROCESSINGML}"><w:abstractNum w:abstractNumId="7"><w:multiLevelType w:val="singleLevel"/><w:lvl w:ilvl="0"><w:start w:val="{start}"/><w:numFmt w:val="decimal"/><w:lvlText w:val="{level_text}"/><w:lvlJc w:val="left"/><w:pPr><w:tabs><w:tab w:val="num" w:pos="720"/></w:tabs><w:ind w:left="720" w:hanging="360"/></w:pPr></w:lvl></w:abstractNum><w:num w:numId="42"><w:abstractNumId w:val="7"/></w:num></w:numbering>"#
        )
        .into_bytes(),
    );
...
fn assert_numbering_is_referenced(docx: &[u8], expected_level_text: &str, expected_start: u32) {
    ...
    assert!(document.contains("<w:ilvl w:val=\"0\"/>"));
    assert!(document.contains("<w:numId w:val=\"42\"/>"));
    assert!(document.contains("same list item body text"));
    ...
    assert!(numbering.contains(&format!("<w:start w:val=\"{expected_start}\"/>")));
    assert!(numbering.contains("<w:numFmt w:val=\"decimal\"/>"));
    assert!(numbering.contains(&format!("<w:lvlText w:val=\"{expected_level_text}\"/>")));
