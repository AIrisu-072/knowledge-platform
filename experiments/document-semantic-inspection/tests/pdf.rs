...
#[test] fn raw_pdf_fixture_builder_is_parser_independent(){let b=support::pdf_fixture::minimal_text_pdf("hello","one");let n=support::pdf_fixture::minimal_text_pdf("hello","two");assert!(b.starts_with(b"%PDF-"));assert_ne!(b,n);assert!(support::pdf_fixture::with_broken_startxref(b).starts_with(b"%PDF-"));assert!(support::pdf_fixture::ambiguous_text_pdf().starts_with(b"%PDF-"));}
#[test] fn producer_and_object_number_noise_is_invariant(){same("pdf/object-id-producer-noise");}
#[test] fn reader_visible_pdf_semantics_change_identity(){for id in ["pdf/text-change","pdf/page-order-change","pdf/link-change","pdf/form-value-change","pdf/image-change"]{different(id);}}
...
#[test] fn ambiguous_text_read_order_fails_closed(){error("pdf/ambiguous-read-order",ErrorCode::UnsupportedSemanticConstruct);}

#[test]
fn pdf_annotations_are_editorial_not_version_identity(){
    same("pdf/annotation-change");
    let result=run_case(&case("pdf/annotation-change"),&PdfAdapter).expect("annotation fixture");
    assert!(result.output.editorial.comments_present);
    assert!(result.output.editorial.comments.iter().any(|comment|comment.content.contains("Note B")));
}
