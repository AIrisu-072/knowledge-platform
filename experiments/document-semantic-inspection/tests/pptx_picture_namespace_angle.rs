...
use document_semantic_inspection_poc::{
    InspectionAdapter, InspectionProfile, PocError, PptxAdapter, fingerprint,
};
...
#[test]
fn redundant_local_drawingml_namespace_on_picture_transform_is_noise() {
...
#[test]
fn redundant_local_drawingml_namespace_on_source_rect_is_noise() {
    let baseline = package_with_asymmetric_image();
    let baseline_output = inspect(&baseline);
    let changed = mutate_picture(&baseline, |picture| {
        replace_exactly_once(
...
#[test]
fn picture_rotation_outside_open_xml_int32_range_fails_closed() {
    let baseline = package_with_asymmetric_image();
    ...
    assert!(
        matches!(
            PptxAdapter.inspect(&overflow, &InspectionProfile::default()),
            Err(PocError::SemanticExtractionFailed(_))
        ),
        "a picture rotation above the signed 32-bit Open XML value range must return a semantic extraction error"
    );
}

fn inspect(package: &[u8]) -> document_semantic_inspection_poc::AdapterOutput {
    PptxAdapter
        .inspect(package, &InspectionProfile::default())
