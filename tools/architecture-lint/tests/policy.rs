...
fn self_hosted_runner_is_rejected() {
    ...
    assert!(
        report
            .findings
            .iter()
            .any(|f| f.code == "ARCH_CI_SELF_HOSTED")
    );
}
...
fn native_windows_runner_is_rejected() {
    ...
    assert!(
        report
            .findings
            .iter()
            .any(|f| f.code == "ARCH_CI_NATIVE_WINDOWS")
    );
}
...
fn dockerfile_variants_are_rejected() {
    ...
    assert!(
        report
            .findings
            .iter()
            .any(|f| f.code == "ARCH_DOCKERFILE_VARIANT")
    );
}
...
fn missing_top_level_permissions_are_rejected_when_required() {
    ...
    assert!(
        report
            .findings
            .iter()
            .any(|f| f.code == "ARCH_CI_PERMISSIONS_MISSING")
    );
}
...
fn workflow_bypassing_mise_is_rejected() {
    ...
    assert!(
        report
            .findings
            .iter()
            .any(|f| f.code == "ARCH_CI_BYPASSES_MISE")
    );
}
