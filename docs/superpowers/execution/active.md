...
Record the supplemental hosted RED above, then repair the in-scope PoC PPTX namespace, unknown-part, and chart-title semantics. Obtain fresh hosted DSI PoC GREEN while production CI still fails only on absent `PptxAdapter`, then implement Task 7 GREEN with `office_oxide 0.1.11` plus an independent bounded raw PresentationML oracle/sentinel. The locally RED same-pixel PNG re-encoding candidate is excluded from Task 7: frozen Design §13.2 requires it for DOCX, while PPTX §13.5 and approved Task 7 do not. Auto-shape geometry/rotation is also held outside the frozen v0 rule. Adding either PPTX rule needs a separately approved Design amendment. Require fresh exact-head standard CI, DSI Sandbox Preflight, and DSI PoC successes to call Task 7 complete. Keep PR #10 Draft and unmerged; no Design amendment is in progress.

## Resume command

> `AIrisu-072/knowledge-platform` のrepositoryとGitHubの現在状態を正本として続行してください。最初に `AGENTS.md`、このActive、Execution Status、Frozen Design、Design approval、承認済みProduction Plan、live branch/PR/CIの順に確認してください。Production Tasks 1–6 COMPLETE。Task 7 initial RED head `452c76d...` はCI `36210857702` がmissing `PptxAdapter`のみで期待どおりFAIL、Sandbox `36210857803` / DSI PoC `36210857696` はSUCCESS。Task 7 supplemental RED head `6f32cbe...` はCI `36212761629` がmissing adapterのみでFAIL、Sandbox `36212761543` SUCCESS、DSI PoC `36212761571` は新規chart-title assertionのみで期待どおりFAILです。証拠をStatusに記録したのでPoC PPTX namespace/unknown-part/chart-titleを修正してPoC GREENを取得し、その後Production Task 7 GREENへ進んでください。PPTX PNG再エンコード同一視とauto-shape geometry/rotation候補はDesign amendment未承認のため除外します。Task 7完了には同一headの3 workflow SUCCESSが必要です。PR #10はOPEN/Draftのままmergeしません。
