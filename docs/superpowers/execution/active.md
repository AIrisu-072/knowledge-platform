...
Independent review passed for the supplemental PPTX XML/package/resource/chart-title/presentation-prefix test-only cases. Commit/push only these in-scope tests and execution records, then obtain exact-head hosted RED before any PoC or production source repair. PoC namespace/unknown-part/chart-title tests failed locally for intended semantic gaps; production focused compilations fail only on missing `PptxAdapter` E0432. The locally RED same-pixel PNG re-encoding candidate is excluded from Task 7: frozen Design §13.2 requires it for DOCX, while PPTX §13.5 and approved Task 7 do not. Auto-shape geometry/rotation is also held outside the frozen v0 rule. Adding either PPTX rule needs a separately approved Design amendment. After the supplemental RED, repair/qualify in-scope PoC semantics and implement Task 7 GREEN with `office_oxide 0.1.11` plus an independent raw PresentationML oracle/sentinel. Require fresh exact-head standard CI, DSI Sandbox Preflight, and DSI PoC successes to call Task 7 complete. Keep PR #10 Draft and unmerged; no Design amendment is in progress.

## Resume command

> `AIrisu-072/knowledge-platform` のrepositoryとGitHubの現在状態を正本として続行してください。最初に `AGENTS.md`、このActive、Execution Status、Frozen Design、Design approval、承認済みProduction Plan、live branch/PR/CIの順に確認してください。Production Tasks 1–6 COMPLETE。Task 7 initial RED headは `452c76d0d92aedb31c4c447cef5bae75388a8b63`、standard CI `36210857702` はmissing `PptxAdapter` E0432のみで期待どおりFAIL、Sandbox `36210857803` / DSI PoC `36210857696` はSUCCESSです。追加PoC XML/chart-titleとproduction package/resource/chart-title/prefix testをin-scope REDとしてcommit/pushし、exact-head evidenceを取得してください。PPTX PNG再エンコード同一視とauto-shape geometry/rotation候補はDesign amendment未承認のため除外します。PoC修正・Production Task 7 GREENへ進み、同一headの3 workflow SUCCESSで完了判定してください。PR #10はOPEN/Draftのままmergeしません。
