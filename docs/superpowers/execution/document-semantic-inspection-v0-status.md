...
- Exact next action: obtain hosted PoC and Sandbox GREEN for the PoC PPTX source promotion being committed with this status. Standard CI is expected to fail only on absent `PptxAdapter`. Then commit bounded production PPTX adapter and require same-head standard CI, Sandbox, and PoC SUCCESS. Keep PR #10 Draft and unmerged.
