use std::io::{Cursor, Read, Write};

use document_semantic_inspection_worker::{AdapterProfile, PptxAdapter, SemanticAdapter};
use quick_xml::{Reader, events::Event};
use zip::{CompressionMethod, ZipArchive, ZipWriter, write::SimpleFileOptions};

const BASE: &[u8] =
    include_bytes!("../../../experiments/document-semantic-inspection/fixtures/pptx/base.pptx");
const SLIDE_PART: &str = "ppt/slides/slide1.xml";
const RELATIONSHIPS_PART: &str = "ppt/slides/_rels/slide1.xml.rels";
const SHAPE_MARKER: &str = r#"<p:cNvPr id="3" name="Shape B"/>"#;
const HYPERLINK_TARGET: &str = "https://example.com/shape-target";
const BASELINE_TOOLTIP: &str = "Open the project";
const MUTANT_TOOLTIP: &str = "Open the document";
const BASELINE_TARGET_FRAME: &str = "_self";
const MUTANT_TARGET_FRAME: &str = "_blank";

#[test]
fn shape_click_hyperlink_tooltip_changes_semantic_identity() {
    assert_attribute_change_changes_identity(
        "tooltip",
        BASELINE_TOOLTIP,
        MUTANT_TOOLTIP,
        BASELINE_TARGET_FRAME,
    );
}

#[test]
fn shape_click_hyperlink_target_frame_changes_semantic_identity() {
    assert_attribute_change_changes_identity(
        "tgtFrame",
        BASELINE_TARGET_FRAME,
        MUTANT_TARGET_FRAME,
        BASELINE_TOOLTIP,
    );
}

fn assert_attribute_change_changes_identity(
    attribute: &str,
    baseline_value: &str,
    mutant_value: &str,
    unchanged_value: &str,
) {
    let (baseline_tooltip, baseline_target_frame, mutant_tooltip, mutant_target_frame) =
        match attribute {
            "tooltip" => (
                baseline_value,
                unchanged_value,
                mutant_value,
                unchanged_value,
            ),
            "tgtFrame" => (
                unchanged_value,
                baseline_value,
                unchanged_value,
                mutant_value,
            ),
            _ => panic!("unsupported hyperlink attribute {attribute}"),
        };

    let baseline = package_with_shape_hyperlink(baseline_tooltip, baseline_target_frame);
    let mutant = package_with_shape_hyperlink(mutant_tooltip, mutant_target_frame);

    assert_valid_zip_and_xml(&baseline);
    assert_valid_zip_and_xml(&mutant);
    assert_shape_hyperlink(&baseline, baseline_tooltip, baseline_target_frame);
    assert_shape_hyperlink(&mutant, mutant_tooltip, mutant_target_frame);
    assert_relationship_target(&baseline, HYPERLINK_TARGET);
    assert_relationship_target(&mutant, HYPERLINK_TARGET);
    assert_only_link_attribute_changed(
        &baseline,
        &mutant,
        baseline_tooltip,
        baseline_target_frame,
        mutant_tooltip,
        mutant_target_frame,
    );

    let profile = AdapterProfile::default();
    let baseline_output = PptxAdapter
        .inspect(&baseline, &profile)
        .expect("valid shape-level hyperlink baseline must inspect");
    let mutant_output = PptxAdapter
        .inspect(&mutant, &profile)
        .expect("valid changed shape-level hyperlink must inspect");

    assert_ne!(
        baseline_output.semantic_fingerprint(),
        mutant_output.semantic_fingerprint(),
        "changing shape-level hyperlink {attribute} changes PPTX semantics under Frozen Design §9.4 and §13.5"
    );
}

fn package_with_shape_hyperlink(tooltip: &str, target_frame: &str) -> Vec<u8> {
    let mut parts = read_parts(BASE);
    let marker = shape_hyperlink_marker(tooltip, target_frame);
    let mut slide_seen = false;
    let mut relationships_seen = false;

    for (name, contents) in &mut parts {
        if name == SLIDE_PART {
            let xml = std::str::from_utf8(contents).expect("base slide XML is UTF-8");
            assert_eq!(xml.matches(SHAPE_MARKER).count(), 1);
            *contents = xml.replace(SHAPE_MARKER, &marker).into_bytes();
            slide_seen = true;
        } else if name == RELATIONSHIPS_PART {
            let xml = std::str::from_utf8(contents).expect("base relationships XML is UTF-8");
            assert!(!xml.contains("rIdShapeLink"));
            let relationship = format!(
                r#"<Relationship Id="rIdShapeLink" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="{HYPERLINK_TARGET}" TargetMode="External"/>"#
            );
            assert_eq!(xml.matches("</Relationships>").count(), 1);
            *contents = xml
                .replace(
                    "</Relationships>",
                    &format!("{relationship}</Relationships>"),
                )
                .into_bytes();
            relationships_seen = true;
        }
    }

    assert!(slide_seen, "base PPTX has the target slide part");
    assert!(
        relationships_seen,
        "base PPTX has the target relationships part"
    );
    write_parts(parts)
}

fn shape_hyperlink_marker(tooltip: &str, target_frame: &str) -> String {
    format!(
        r#"<p:cNvPr id="3" name="Shape B"><a:hlinkClick r:id="rIdShapeLink" tooltip="{tooltip}" tgtFrame="{target_frame}"/></p:cNvPr>"#
    )
}

fn assert_shape_hyperlink(archive: &[u8], tooltip: &str, target_frame: &str) {
    let slide = String::from_utf8(read_part(archive, SLIDE_PART)).expect("slide XML is UTF-8");
    assert_eq!(
        slide
            .matches(&shape_hyperlink_marker(tooltip, target_frame))
            .count(),
        1
    );
}

fn assert_relationship_target(archive: &[u8], target: &str) {
    let relationships = String::from_utf8(read_part(archive, RELATIONSHIPS_PART))
        .expect("relationships XML is UTF-8");
    let expected = format!(
        r#"<Relationship Id="rIdShapeLink" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="{target}" TargetMode="External"/>"#
    );
    assert_eq!(relationships.matches(&expected).count(), 1);
}

fn assert_only_link_attribute_changed(
    baseline: &[u8],
    mutant: &[u8],
    baseline_tooltip: &str,
    baseline_target_frame: &str,
    mutant_tooltip: &str,
    mutant_target_frame: &str,
) {
    let baseline_parts = read_parts(baseline);
    let mutant_parts = read_parts(mutant);
    assert_eq!(
        baseline_parts
            .iter()
            .map(|(name, _)| name)
            .collect::<Vec<_>>(),
        mutant_parts
            .iter()
            .map(|(name, _)| name)
            .collect::<Vec<_>>(),
        "the mutant keeps the same ZIP entries in the same order"
    );

    let changed_parts = baseline_parts
        .iter()
        .zip(&mutant_parts)
        .filter_map(|((name, original), (_, changed))| {
            (original != changed).then_some(name.as_str())
        })
        .collect::<Vec<_>>();
    assert_eq!(changed_parts, [SLIDE_PART]);

    let baseline_slide =
        String::from_utf8(read_part(baseline, SLIDE_PART)).expect("baseline slide XML is UTF-8");
    let mutant_slide =
        String::from_utf8(read_part(mutant, SLIDE_PART)).expect("mutant slide XML is UTF-8");
    assert_eq!(
        baseline_slide.replace(
            &shape_hyperlink_marker(baseline_tooltip, baseline_target_frame),
            &shape_hyperlink_marker(mutant_tooltip, mutant_target_frame),
        ),
        mutant_slide,
        "only the selected a:hlinkClick attribute value changes"
    );
}

fn assert_valid_zip_and_xml(archive: &[u8]) {
    let mut zip = ZipArchive::new(Cursor::new(archive)).expect("valid PPTX ZIP");
    for index in 0..zip.len() {
        let mut entry = zip.by_index(index).expect("central-directory entry");
        let name = entry.name().to_owned();
        let mut contents = Vec::new();
        entry
            .read_to_end(&mut contents)
            .expect("PPTX entry data and CRC are valid");
        if name.ends_with(".xml") || name.ends_with(".rels") {
            let xml = std::str::from_utf8(&contents).expect("PPTX XML is UTF-8");
            let mut reader = Reader::from_str(xml);
            reader.config_mut().check_end_names = true;
            loop {
                match reader.read_event() {
                    Ok(Event::Eof) => break,
                    Ok(_) => {}
                    Err(error) => panic!("PPTX part {name} is malformed XML: {error}"),
                }
            }
        }
    }
}

fn read_parts(archive: &[u8]) -> Vec<(String, Vec<u8>)> {
    let mut zip = ZipArchive::new(Cursor::new(archive)).expect("valid PPTX ZIP");
    let mut parts = Vec::with_capacity(zip.len());
    for index in 0..zip.len() {
        let mut entry = zip.by_index(index).expect("central-directory entry");
        let name = entry.name().to_owned();
        let mut contents = Vec::new();
        entry
            .read_to_end(&mut contents)
            .expect("PPTX entry and CRC");
        parts.push((name, contents));
    }
    parts
}

fn read_part(archive: &[u8], name: &str) -> Vec<u8> {
    read_parts(archive)
        .into_iter()
        .find(|(part_name, _)| part_name == name)
        .unwrap_or_else(|| panic!("missing PPTX part {name}"))
        .1
}

fn write_parts(parts: Vec<(String, Vec<u8>)>) -> Vec<u8> {
    let mut writer = ZipWriter::new(Cursor::new(Vec::new()));
    let options = SimpleFileOptions::default().compression_method(CompressionMethod::Stored);
    for (name, contents) in parts {
        writer
            .start_file(name, options)
            .expect("start PPTX ZIP part");
        writer.write_all(&contents).expect("write PPTX ZIP part");
    }
    writer.finish().expect("finish PPTX ZIP").into_inner()
}
