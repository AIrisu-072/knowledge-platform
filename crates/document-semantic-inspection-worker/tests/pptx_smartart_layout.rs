...
const LAYOUT_LEFT_TO_RIGHT: &str = r#"<?xml version="1.0" encoding="UTF-8"?>
...
const LAYOUT_TOP_TO_BOTTOM: &str = r#"<?xml version="1.0" encoding="UTF-8"?>
<dgm:layoutDef xmlns:dgm="http://schemas.openxmlformats.org/drawingml/2006/diagram">
  <dgm:layoutNode name="diagram">
...
#[test]
fn smartart_layout_direction_changes_identity() {
    let baseline = qualified_smartart_package(LAYOUT_LEFT_TO_RIGHT);
    let mutant = qualified_smartart_package(LAYOUT_TOP_TO_BOTTOM);
    ...
    assert!(
        String::from_utf8(read_part(&baseline, LAYOUT_PART))
            .expect("layout part is UTF-8")
            .contains("linDir\" val=\"fromL\"")
    );
    assert!(
        String::from_utf8(read_part(&mutant, LAYOUT_PART))
            .expect("mutant layout part is UTF-8")
            .contains("linDir\" val=\"fromT\"")
    );

    let profile = AdapterProfile::default();
    let baseline_output = PptxAdapter
        .inspect(&baseline, &profile)
        .expect("qualified data+layout SmartArt baseline must be accepted");
    let mutant_output = PptxAdapter
        .inspect(&mutant, &profile)
        .expect("qualified layout-only SmartArt mutant must be accepted");

    // DrawingML `linDir` controls whether child shapes flow horizontally or vertically.
    assert_ne!(
        baseline_output.semantic_fingerprint(),
        mutant_output.semantic_fingerprint(),
        "changing linDir from left-to-right to top-to-bottom changes visible SmartArt arrangement"
    );
}
