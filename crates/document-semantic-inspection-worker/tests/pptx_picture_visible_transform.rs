...
#[test]
fn pptx_picture_source_crop_changes_visible_image_identity() {
...
#[test]
fn pptx_picture_rotation_changes_visible_image_identity() {
    let baseline = package_with_asymmetric_image();
    let baseline_pixels = decoded_pixels(&read_part(&baseline, IMAGE));
    assert_eq!(baseline_pixels, (2, 1, [RED, BLUE].concat()));
...
#[test]
fn pptx_picture_horizontal_flip_changes_visible_image_identity() {
    let baseline = package_with_asymmetric_image();
    let baseline_pixels = decoded_pixels(&read_part(&baseline, IMAGE));
    assert_eq!(baseline_pixels, (2, 1, [RED, BLUE].concat()));
