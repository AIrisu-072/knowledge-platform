...
#[test]
fn pptx_missing_package_root_relationships_fail_closed() {
...
#[test]
fn pptx_package_root_office_document_relationship_must_target_presentation_part() {
    assert_baseline_accepted();
...
#[test]
fn pptx_duplicate_package_root_office_document_relationships_fail_closed() {
    assert_baseline_accepted();
...
#[test]
fn pptx_character_reference_serialization_of_content_type_preserves_fingerprint() {
    assert_baseline_accepted();
    ...
    let changed = PptxAdapter
        ...
        .expect("XML character reference in a ContentType attribute must inspect");
    assert_eq!(
